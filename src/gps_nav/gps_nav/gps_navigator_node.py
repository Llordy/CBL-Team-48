...
ANGULAR_P_GAIN      = 0.4       # proportional gain for heading error
...
class GpsNavigatorNode(Node):
    ...
    def _execute_cb(self, goal_handle):
        ...
        # Just block here waiting for the goal to finish
        # The actual work happens in _control_loop_cb
        while rclpy.ok() and self._active_goal is not None:
            time.sleep(0.05)
        result = GpsGoal.Result()
        result.success = self._last_result_success
        result.message = self._last_result_message
        return result

    def _control_loop_cb(self):
        """Called by the node timer at CONTROL_HZ — never blocks."""
        self.get_logger().debug("starting loop callback")
        ...
        if goal_handle.is_cancel_requested:
            ...
            self._finish_goal(False, 'Cancelled.')
            return
        ...
        dist = haversine_distance(cur.latitude, cur.longitude,
                                  goal.latitude, goal.longitude)
        target_bearing = bearing_to(cur.latitude, cur.longitude,
                                    goal.latitude, goal.longitude)
        ...
        if dist <= radius:
            self._publish_stop()
            goal_handle.succeed()
            self._finish_goal(True, f'Arrived within {dist:.2f} m.')
            return

        feedback_msg = GpsGoal.Feedback()
        feedback_msg.distance_to_goal = dist
        feedback_msg.bearing_to_goal = target_bearing
        feedback_msg.state = 'PAUSED' if self._paused else f'NAVIGATING ...'
        ...
        self.get_logger().info(f"turn speed: {angular_z}")
        approach_speed = 0.0
        if abs(heading_error) < HEADING_ALIGN_DEG:
            approach_speed = min(MAX_LINEAR_SPEED, dist * LINEAR_P_GAIN)
            self.get_logger().info(f"go_forth: {approach_speed}")

        self.get_logger().info(f"compute time: {time.monotonic() - start}s");

        self._publish_cmd(approach_speed, angular_z)

    def _finish_goal(self, success, message):
